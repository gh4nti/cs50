...
def index():
    ...
    for stock in stocks: total += stock["price"] * stock["totalShares"]
...
def buy():
    if(request.method == "POST"):
        ...
        if not symbol: return apology("Please enter a symbol.")
        elif not item: return apology("Invalid symbol.")

        try: shares = int(request.form.get("shares"))
        except: return apology("Shares must be an integer.")

        if shares <= 0: return apology("Shares must be positive.")
        ...
        if cash < total_price: return apology("Not enough cash.")
        else:
...
def quote():
    if(request.method == "POST"):
        ...
        if not symbol: return apology("Please enter a symbol.")
        item = lookup(symbol)
        if not item: return apology("Invalid symbol.")
...
def register():
    if(request.method == "POST"):
        ...
        if not username: return apology("Please enter username.")
        elif not password: return apology("Please enter password.")
        elif not confirmation: return apology("Please confirm password.")

        if password != confirmation: return apology("Passwords do not match.")
        ...
        try:
            ...
            return redirect("/")
        except: return apology("Username already exists.")
...
def sell():
    ...
    if request.method == "POST":
        ...
        if shares_owned < shares: return apology("You don't have enough shares.")
